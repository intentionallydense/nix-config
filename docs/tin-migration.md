...
- **Invidious DC-IP verdict (2026-06-12): blocked.** Companion generates a potoken but validation fails — googlevideo returns non-200 for all stream formats from the Hetzner IP. Deployed + DB-migrated on tin but playback-dead; carbon's instance stays Yattee's backend. Fix path: companion `networking.proxy` → wireproxy/Mullvad once the WG key arrives (shared with the slskd plan); fallback: logged-in cookies (account-linked, throwaway acct advised) or keep invidious home-hosted in Phase 2.

## 2026-06-12 follow-up 3 (Mullvad egress)
- **wireproxy module live** (modules/server/mullvad-egress): one tunnel → ch-zrh-wg-005 (Zurich), SOCKS5 :1080 + HTTP :8888. Key = sops `wireproxy/tin` ("Classy Boar"). Two false starts: (1) germanium committed the key on a stale, tin-less recipient set — caught before it clobbered main; the correct 3-recipient re-key is on main. (2) first key value was a revoked Mullvad key (silent no-handshake) — re-set to the live key, handshake confirmed.
- **slskd: DONE.** Soulseek login now exits Zurich ("Using Proxy 127.0.0.1:1080" → logged in; am.i.mullvad confirms 193.32.127.250). Proxy-only by config = no raw-IP leak. Unconnectable (no Mullvad port-forward) — sharing is best-effort, accepted.
- **invidious: still blocked.** Proxy verified exiting Zurich, but googlevideo rejects the Mullvad range too — potoken validation gets the same non-200 as the raw Hetzner IP. Generation works, feed/channel endpoints work (200), playback dead. Only remaining self-host fix: logged-in YouTube session (companion cookies/oauth, throwaway acct) — Phase 2. Yattee stays on carbon until then. ⚠ This means invidious does NOT survive carbon's reinstall as a working player — decide in Phase 2 whether to do cookies-on-tin or re-home it residential.
